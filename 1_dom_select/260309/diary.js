...
function renderEntry(entry) {
  ...
  const emojis_span = document.createElement("span");
  emojis_span.className = "entry-mood";
  emojis_span.textContent = MOOD_EMOJIS[entry.mood];
  ...
  date_span.className = "entry-date";
  date_span.textContent = entry.date;

  header.appendChild(emojis_span);
...
function updateEntry(id, updates) {
  ...
  const entry = diary.find((find_diary) => {
    return find_diary.id === id;
  });

  if (entry) {
    return Object.assign(entry, updates);
    //Object.assign()은 Object 생성자에 붙어 있는 정적 함수
    //정적 메서드 : Object에 붙어 있음, 객체 인스턴스에 붙어있지 않음 따라서 체이닝 불가능
  } //전역 함수라서 따로 변수에 담아줘야함
  //체이닝은 : 앞 단계 반환값이 다음 메서드를 가지고 있을 때만 가능하다.
}
...
function fillFormForEdit(id) {
  ...
  const element = diary.find((element) => {
...
function deleteEntry(id) {
  ...
  const index = diary.findIndex((element) => {
    return element.id === id;
  });

  if (index === -1) return;

  diary.splice(index, 1); //splice() 는 배열을 직접 수정하는 메서드임
  //index= 시작 인덱스 1= 몇 개 삭제할건지
  renderStats();
  renderAllEntries();
}
...
function renderStats() {
  ...
  stats_container.className = "stats-grid";

  const stats_items = document.createElement("p");
  stats_items.className = "stat-total";
  stats_items.textContent = `전체 일기: ${stats.total}개`;

  stats_container.appendChild(stats_items);

  for (let mood in stats.moods) {
    const item = document.createElement("p");
    item.className = "stat-item";

    const stats_emoji = MOOD_EMOJIS[mood];
    const stats_count = stats.moods[mood];

    item.textContent = `${stats_emoji} ${mood} : ${stats_count}`;

    stats_container.appendChild(item);
  }
  statsContainer.appendChild(stats_container);
}
